...
| 9 — instructors hidden | done. `migrations/004` adds `is_instructor`; 1075 박홍기 and 1080 유현규 set. Both member queries filter `status = 'active' AND NOT is_instructor`, and the flag is editable in Manage Members. /members renders 54 of 65. |
| 10 — API auth (Phase 2) | done, pending deploy. `requireAuth`/`authFetch` verify Firebase ID tokens server-side on all six admin routes, now with `role: 'admin'`. `migrations/005` adds `centuryusers`; Google sign-in gated by `INVITE_CODE`. |
